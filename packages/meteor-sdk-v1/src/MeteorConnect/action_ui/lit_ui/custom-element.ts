...
export const customElement =
  <T extends CE>(tagName: string) =>
  (classTarget: T): void | T => {
    ...
    if (existing) {
      ...
      const finalizeStyles =
        (existing as any).finalizeStyles ?? (classTarget as any).finalizeStyles;
      if (finalizeStyles) {
        (existing as any)._styles = undefined;
        (existing as any).elementStyles = finalizeStyles.call(
          existing,
          (existing as any).styles ?? [],
        );
      }
      ...
      document.querySelectorAll<LitElement>(tagName).forEach((el) => {
        ...
        if (el.shadowRoot && Array.isArray(ctor.elementStyles)) {
          const sheets = ctor.elementStyles.map((s: any) => s?.styleSheet).filter(Boolean);
