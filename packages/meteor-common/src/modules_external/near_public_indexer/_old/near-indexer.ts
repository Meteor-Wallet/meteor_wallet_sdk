...
const poolMatch = (network: string) =>
...
const findStakingDeposits = async (ctx) => {
  const { accountId, network } = ctx.params;
    ...
    `
        with deposit_in as (
            ...
            where
                ...
                receipt_receiver_account_id like ANY(ARRAY${poolMatch(network)})
            ...
            where
                ...
                receipt_predecessor_account_id like ANY(ARRAY${poolMatch(network)})
...
const findLikelyTokens = async (ctx) => {
  ...
  const { rows } = await pool.query(
    [received, mintedWithBridge, calledByUser, ownershipChangeEvents].join(" union "),
    [accountId, BRIDGE_TOKEN_FACTORY_ACCOUNT_ID],
  );
  ctx.body = rows.map(({ receiver_account_id }) => receiver_account_id);
};
...
const validatorCache = new Cache({
  stdTTL: 60,
  checkperiod: 0,
  useClones: false,
});
...
async function fetchAndCacheValidators(cache) {
  const { rows: validatorDetails } = await pool.query(
    `SELECT account_id FROM accounts WHERE account_id LIKE ANY(ARRAY${poolMatch})`,
  );
