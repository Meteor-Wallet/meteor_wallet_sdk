...
class JSBI extends Array {
  ...
  static __fromString(string: string, radix: number = 0): JSBI | null {
    ...
    if ((radix & (radix - 1)) === 0) {
      ...
      do {
        ...
        const digitsSoFar =
          (((bitsPerChar * charsSoFar + roundup) >>> JSBI.__kBitsPerCharTableShift) / 30) | 0;
  ...
  __setHalfDigit(i: number, value: number): void {
    ...
    const updated =
      i & 1 ? (previous & 0x7fff) | (value << 15) : (previous & 0x3fff8000) | (value & 0x7fff);
