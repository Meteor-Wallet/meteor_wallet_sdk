...
- **App ids per action:** today the session sends `meteorAppIds: [this.input.meteorAppId]` (mobile app). Transfer must target the web wallet now and more apps later. Add to `IMeteorConnectMobileBridgeConfig`:
  ...
  Default derivation: when `config.meteorAppId` is the `_dev` mobile id → `[meteor_wallet_web_dev]`, otherwise → `[meteor_wallet_web]` — matching meteor-frontend, which identifies env-based with exactly these ids (§2.4). Ordering is link-selection preference; the list grows (rather than switches) when Meteor Mobile ships transfer support (§14.1). Integrations testing against the `mc_backend` demo wallet can override with `meteorAppIds: [meteor_bridge_test_web]`.
